<template>
	<div v-if="isLogged && getUser !== null && getUser.roles.includes('AGENT')" class="md-layout md-alignment-top-center">
		...
		<md-table v-model="pricelistItems" md-sort="name" md-sort-order="asc" md-card class="md-layout-item md-size-80 md-small-size-150">
			...
			<md-table-empty-state md-label="No Price List Items" :md-description="`There is not yet price list items added to ad your fist click on button below`">
...
export default {
	...
	mounted() {
		if (this.getUser) {
			this.$store.dispatch("getUsersPriceList", this.getUser.id);
		}
	},
	data() {
		return {
	...
	computed: {
		...mapGetters(["getUser", "getPriceList", "isLogged", "getDiscount"]),
		pricelistItems: {
			get() {
