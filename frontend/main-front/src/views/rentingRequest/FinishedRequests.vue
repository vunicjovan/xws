<template>
	<div id="component">
		...
		<div class="md-layout md-gutter md-alignment-center">
			...
			<md-table v-model="requests" md-card class="md-layout-item md-size-50 md-small-size-100" v-if="requests.length != 0">
				<md-table-toolbar>
				...
				<md-table-row v-if="requests.length !== 0" slot="md-table-row" slot-scope="{ item }">
					<md-table-cell md-label="Vehicle">{{ item.vehicle }}</md-table-cell>
					<md-table-cell md-label="Interval">{{ item.startDate }} - {{ item.endDate }}</md-table-cell>
...
export default {
	...
	components: {
		RentingReports: () => import("../../components/renting/RentingReports.vue"),
	},
	data() {
		return {
			active: false,
	...
	mounted() {
		if (this.getUser) this.$store.dispatch("getFinishedRentingRequests", this.getUser.id);
	},
	computed: {
		...mapGetters(["getFinishedRequests", "getUser"]),
		requests: {
			get() {
	...
	methods: {
		...
		submitReport() {
			this.$store
				.dispatch("newRentingReport", this.form)
				.then((data) => {
					this.active = false;
					this.$store.dispatch("getFinishedRentingRequests", this.getUser.id);
					this.$store.dispatch("getRentingReports", this.getUser.id);
				})
	...
	watch: {
		getUser: {
			handler(val) {
				if (val) this.$store.dispatch("getFinishedRentingRequests", val.id);
			},
		},
	},
